...
import json
import re
from inventory import hosts, links, anysecs, icmp_types
from flask import Flask, render_template, jsonify, request
from pygnmi.client import gNMIclient, telemetryParser, gNMIException
import subprocess
import signal
                ...
                for router, port in routers_data.items():
                    if router in self.routers:
                        if port in self.routers[router]["ports"]:
                            if self.routers[router]["ports"][port]["admin-state"]=="enable":
                    ...
                    if router in self.routers:
                        if "anysec_group" in self.routers[router]:
                            if anysec_group in self.routers[router]["anysec_group"]:
                                if self.routers[router]["anysec_group"][anysec_group]["admin-state"]=="enable":
        ...
        with gNMIclient(target=(host_entry["hostname"], host_entry["port"]),username=host_entry["username"], password=host_entry["password"], insecure=True) as gc:
            ...
            for telemetry_entry in telemetry_stream:
                ...
                if "port-id" in telemetry_entry["update"]["prefix"]:
                    port=re.findall('port-id=(.*)',re.findall('\[(.*?)\]', telemetry_entry["update"]["prefix"])[0])[0]
                    ...
                    self.routers[host_entry["hostname"]]["ports"][port]["admin-state"]=telemetry_entry["update"]["update"][0]["val"]
                if "anysec" in telemetry_entry["update"]["prefix"]:
                    anysec_group=re.findall('group-name=(.*)',re.findall('\[(.*?)\]', telemetry_entry["update"]["prefix"])[0])[0]
                    if host_entry["hostname"] not in self.routers:
                        self.routers[host_entry["hostname"]]={}
                    if "anysec_group" not in self.routers[host_entry["hostname"]]:
                        self.routers[host_entry["hostname"]]["anysec_group"]={}
                    if anysec_group not in self.routers[host_entry["hostname"]]["anysec_group"]:
                        self.routers[host_entry["hostname"]]["anysec_group"][anysec_group]={}
                    self.routers[host_entry["hostname"]]["anysec_group"][anysec_group]["admin-state"]=telemetry_entry["update"]["update"][0]["val"]
    ...
    def update_port_status(self,host_entry):
        ...
        with gNMIclient(target=(host_entry["hostname"], host_entry["port"]),username=host_entry["username"], password=host_entry["password"], insecure=True) as gc:
            ...
            for port_result in result["notification"][0]["update"]:
                ...
                port=re.findall('port-id=(.*)',re.findall('\[(.*?)\]', port_result["path"])[0])[0]
                if host_entry["hostname"] not in self.routers:
                    self.routers[host_entry["hostname"]]={}
                if "ports" not in self.routers[host_entry["hostname"]]:
                    self.routers[host_entry["hostname"]]["ports"]={}
            ...
            try:
                with gNMIclient(target=(host_entry["hostname"], host_entry["port"]),username=host_entry["username"], password=host_entry["password"], insecure=True) as gc:
                    ...
                    for anysec_result in result["notification"][0]["update"]:
                        anysec_group=re.findall('group-name=(.*)',re.findall('\[(.*?)\]', anysec_result["path"])[0])[0]
                        if host_entry["hostname"] not in self.routers:
                            self.routers[host_entry["hostname"]]={}
                        if "anysec_group" not in self.routers[host_entry["hostname"]]:
                            self.routers[host_entry["hostname"]]["anysec_group"]={}
                        if anysec_group not in self.routers[host_entry["hostname"]]["anysec_group"]:
                            self.routers[host_entry["hostname"]]["anysec_group"][anysec_group]={}
                        self.routers[host_entry["hostname"]]["anysec_group"][anysec_group]["admin-state"]=anysec_result["val"]
...
def execute_gnmic_link_toggle(link_name):
    try:
        ...
        if link_name not in telemetry.links:
            return '{status:"failed", message:"The link '+link_name+' is not configured"}'
        action=""
...
def execute_gnmic_anysec_toggle(anysec_name):
    try:
        ...
        if anysec_name not in telemetry.anysecs:
            return '{status:"failed", message:"The anysec '+anysec_name+' is not configured"}'
        action=""
...
def execute_gnmic_get_port_status(element, card, mda, conector, port='1'):
    try:
        ...
        request = subprocess.run('gnmic -a '+element+':57400 -u admin -p admin --insecure get --path /configure/port[port-id='+portID+']/admin-state', shell=True, capture_output=True, text=True)
