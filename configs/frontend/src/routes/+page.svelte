<script lang="ts">
  ...
  async function setState(module: string, service: string, currentState: boolean, targetState: string) {
    try {
      const url = baseUrl + `/set/${module}/${service}/${targetState}`
...
<!-- SIDEBAR -->
<div id="sidebar" class="fixed h-screen overflow-hidden transform transition ease-in-out duration-300">
  <aside class="text-sm pb-4 overflow-y-auto scroll-light dark:scroll-dark z-20 w-[300px] h-screen bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700">
...
<div class="pt-[55px]">
  ...
  <iframe class="h-[calc(100vh-55px)] w-screen" src="http://{urlHost}:3000/d/kawVPD-Gk/anysec-telemetry"></iframe>
</div>